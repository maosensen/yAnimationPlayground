...
> **Skill missing or stale?** Run `pnpm exec hyperframes skills update <name>` to install/refresh
> the specific skill you need (the `/hyperframes` router does this automatically before
> entering a workflow), or bare `pnpm exec hyperframes skills update` to refresh the core set plus
...
```bash
pnpm dev          # start the preview server (long-running — keep it alive in background)
pnpm check        # lint + runtime + layout + motion + contrast (one command)
pnpm render       # render to MP4
pnpm publish      # publish and get a shareable link
pnpm exec hyperframes lint --verbose  # include info-level findings
pnpm exec hyperframes lint --json     # machine-readable output for CI
pnpm exec hyperframes docs <topic> # reference docs in terminal
```

> **`pnpm dev` is a long-running server, not a one-shot command.** It blocks until stopped.
> In Claude Code, always run it with `run_in_background: true`. Never run it as a foreground
> command — it will time out and the server will die, breaking the browser preview.

> **Pinned CLI version.** This composition uses the exact HyperFrames version owned by the parent workspace. Upgrade that dependency intentionally, then run `pnpm exec hyperframes upgrade --project . --check` before applying the migration.
...
```bash
pnpm exec hyperframes docs <topic>
```
